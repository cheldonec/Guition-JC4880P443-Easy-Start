...
static esp_err_t check_partition_has_firmware(const esp_partition_t* partition)
{
	...
	ESP_LOGI(TAG, "Partition address: 0x%" PRIx32 ", size: %" PRIu32, partition->address, partition->size);
	...
	while (total_checked < 1024 && total_checked < partition->size) {
		...
		if (ret != ESP_OK) {
			ESP_LOGE(TAG, "Failed to read partition data for validation at offset %u: %s",
			         (unsigned int)total_checked, esp_err_to_name(ret));
			return ret;
		}

		// 🔍 Добавлено: логируем первые 16 байт
		if (total_checked == 0) {
			ESP_LOGI(TAG, "First 16 bytes of partition (offset %u):", (unsigned int)total_checked);
			ESP_LOG_BUFFER_HEX_LEVEL(TAG, buffer, 16, ESP_LOG_INFO);
		}
...
static esp_err_t parse_image_header(const esp_partition_t* partition, size_t* firmware_size, char* app_version_str, size_t version_str_len)
{
	...
	ESP_LOGI(TAG, "Reading image header from partition at offset 0...");
	...
	// 🔍 Логируем загруженный заголовок
	ESP_LOGI(TAG, "Image header read:");
	ESP_LOG_BUFFER_HEX_LEVEL(TAG, &image_header, sizeof(image_header), ESP_LOG_INFO);
	...
	*firmware_size = total_size;
	...
	if (ret == ESP_OK) {
		ESP_LOGI(TAG, "Last 48 bytes of image (offset %u):", (unsigned int)tail_offset);
		ESP_LOG_BUFFER_HEX_LEVEL(TAG, tail_data, 48, ESP_LOG_INFO);
	}
...
esp_err_t ota_partition_perform(const char* partition_label)
{
	...
	ESP_LOGI(TAG, "Found partition: %s, size: %" PRIu32 " bytes, address: 0x%" PRIx32,
	...
	while (offset < firmware_size) {
		...
		if (ret != ESP_OK) {
			ESP_LOGE(TAG, "Failed to read partition at offset %u: %s", (unsigned int)offset, esp_err_to_name(ret));
			esp_hosted_slave_ota_end();
			return ESP_HOSTED_SLAVE_OTA_FAILED;
		}

		// 🔍 Логируем первые 32 байта каждого чанка
		if (chunk_count == 0) {
			ESP_LOGI(TAG, "First OTA chunk (offset %u, size %u):", (unsigned int)offset, (unsigned int)bytes_read);
			ESP_LOG_BUFFER_HEX_LEVEL(TAG, chunk, bytes_read > 32 ? 32 : bytes_read, ESP_LOG_INFO);
		}
